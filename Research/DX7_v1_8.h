...
uint8_t& m_patch_current_lfo_amp_mod_depth   = m_patch_current_buffer[0x8C];
uint8_t& m_patch_current_lfo_sync            = m_patch_current_buffer[0x8D];
uint8_t& m_patch_current_lfo_waveform        = m_patch_current_buffer[0x8E];
uint8_t& m_patch_current_pitch_mod_sens      = m_patch_current_buffer[0x8F];
uint8_t& m_patch_current_transpose           = m_patch_current_buffer[0x90];
uint8_t* m_patch_current_name                = &m_patch_current_buffer[0x91];
uint8_t* m_patch_current_name_last_char      = &m_patch_current_buffer[0x9A];
uint8_t* m_patch_current_ops_on_off          = &m_patch_current_buffer[0x9B];
...
uint16_t m_lfo_phase_increment {0};
uint16_t m_lfo_delay_increment {0};
uint8_t  m_lfo_waveform {0};
uint8_t  m_lfo_pitch_mod_depth {0};
uint8_t  m_lfo_amp_mod_depth {0};
uint8_t  m_lfo_pitch_mod_sens {0};
...
// ; ==============================================================================
//
void patch_deserialise(uint8_t* ptr)
{
// PATCH_DESERIALISE:
//     STD     <M_COPY_SRC_PTR
//     JSR     DELAY
...
//     INX
//     STX     <M_COPY_DEST_PTR
      uint8_t kbd_lvl_scl = *m_copy_src_ptr++;
      *m_copy_dest_ptr++ = (kbd_lvl_scl >> 0) & 0b11;
      *m_copy_dest_ptr++ = (kbd_lvl_scl >> 2) & 0b11;
...
//     INX
//     STX     <M_COPY_DEST_PTR
      uint8_t kbd_rate_osc_detune = *m_copy_src_ptr++;
      *m_copy_dest_ptr = kbd_rate_osc_detune & 0b111;
      m_copy_dest_ptr[7] = kbd_rate_osc_detune >> 3;
      ++m_copy_dest_ptr;
...
//     INX
//     STX     <M_COPY_DEST_PTR
      uint8_t amp_mod_sens_key_vel_sense = *m_copy_src_ptr++;
      *m_copy_dest_ptr++ = amp_mod_sens_key_vel_sense & 0b11;
      *m_copy_dest_ptr++ = amp_mod_sens_key_vel_sense >> 2;
...
//     INX
//     STX     <M_COPY_DEST_PTR
      uint8_t osc_bits = *m_copy_src_ptr++;
      *m_copy_dest_ptr++ = osc_bits & 0b1;
      *m_copy_dest_ptr++ = osc_bits >> 1;
...
//     INX
//     STX     <M_COPY_DEST_PTR
   uint8_t alg_bits = *m_copy_src_ptr++;
   *m_copy_dest_ptr++ = alg_bits & 0b111;
   *m_copy_dest_ptr++ = alg_bits >> 3;
...
//     INX
//     STX     <M_COPY_DEST_PTR
   uint8_t lfo_bits = *m_copy_src_ptr++;
...
//     FCB $FF, $FF, $FF, $FF, $FF
//     FCB $FF, $FF, $FF
const uint8_t table_kbd_scaling_curve_exp[36] =
{
   0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
   0x08, 0x09, 0x0B, 0x0E, 0x10, 0x13, 0x17, 0x1C,
   0x21, 0x27, 0x2F, 0x39, 0x43, 0x50, 0x5F, 0x71,
   0x86, 0xA0, 0xBE, 0xE0, 0xFF, 0xFF, 0xFF, 0xFF,
   0xFF, 0xFF, 0xFF, 0xFF
};
//
// ; ==============================================================================
...
//     FCB $E8, $F0, $F8, $FF, $FF
//     FCB $FF, $FF
const uint8_t table_kbd_scaling_curve_lin[36] =
{
   0x00, 0x08, 0x10, 0x18, 0x20, 0x28, 0x30, 0x38,
   0x40, 0x48, 0x50, 0x58, 0x60, 0x68, 0x70, 0x78,
   0x80, 0x88, 0x90, 0x98, 0xA0, 0xA8, 0xB2, 0xB8,
...
uint16_t patch_load_quantise_lfo_speed(uint8_t in)
{
   ...
   uint8_t scaled_speed = (in * 660) >> 8;
   if (scaled_speed < 160)
   {
      // LFO speed 1..62 => 22..1749
      return 11 * scaled_speed;
   }
...
uint16_t patch_load_quantise_lfo_delay(uint8_t in)
{
   ...
   // Treat as a 7-bit floating point value _EEEMMMM
...
//     STAA    M_MIDI_ACTV_SENS_TX_TRIGGER
   if (++m_midi_actv_sens_tx_cntr > 50)
   {
       m_midi_actv_sens_tx_cntr    = 0;
       m_midi_actv_sens_tx_trigger = 1;
   }
//
...
//     STD     <M_LFO_DELAY_COUNTER
   ...
   if (delay > 0xFFFF)
   {
      ...
      lfo_fade_in += m_lfo_delay_increment < 0x100 ? 1 : (m_lfo_delay_increment >> 8);
      if (lfo_fade_in > 0xFF)
      {
         lfo_fade_in = 0xFF;
      }
      m_lfo_fade_in_scale_factor = uint8_t(lfo_fade_in);
...
//     STD     <M_LFO_PHASE_ACCUMULATOR
   ...
   phase += m_lfo_phase_increment;
   if (phase > 0xFFFF)
   {
      m_lfo_sample_hold_reset_flag = 1 << 7;
   }
   else
   {
      m_lfo_sample_hold_reset_flag = 0;
   }
   m_lfo_phase_accumulator = uint16_t(phase);
...
//     BRA     _STORE_AND_EXIT
   if (m_lfo_waveform == 0)
   {
      m_lfo_tri_accumulator = phase >> 8;
      phase = phase << 1;
      uint8_t lfo_amp = phase >> 8;
      if (m_lfo_tri_accumulator > 0x7f)
      {
         lfo_amp = ~lfo_amp;
      }
...
//     BNE     _IS_LFO_WAVE_2?
   else if (m_lfo_waveform == 4)
   {
      uint8_t index = (m_lfo_phase_accumulator >> 8) % 64;
      if (m_lfo_phase_accumulator & 0x4000)
      {
         index ^= 63;
      }
      uint8_t output = table_lfo_sin[index];
      if (m_lfo_phase_accumulator & 0x8000)
      {
         output = ~output;
      }
      m_lfo_curr_amplitude = output;
   }
//
...
//     TBA
   ...
   {
      if (m_lfo_sample_hold_reset_flag > 0x7F)
      {
         m_lfo_sample_hold_accumulator = m_lfo_sample_hold_accumulator * 179 + 11;
